...
| **criminalActivities** (1.4k) | Store-robbery subsystem: definitions, robbery/cashier state machines, seeded loot, unsecured proceeds + receipts, alarm→crime bridge, threat director, validation, persistence, markers | `activityDefinitions.ts`, `robberyEngine.ts`, `robberyLogic.ts`, `activityRuntime.ts`, `activityBridge.ts`, `ActivityDirector.tsx` | [CRIMINAL_ACTIVITIES](CRIMINAL_ACTIVITIES.md) |
| **interiors** (0.9k) | Reusable interior registry (apartment + store interiors), enter/exit teleport flow, store scene/colliders/cashier/customers | `interiorRegistry.ts`, `ApartmentInterior.tsx`, `StoreInteriors.tsx`, `apartmentLayout.ts` | [CRIMINAL_ACTIVITIES](CRIMINAL_ACTIVITIES.md) |
