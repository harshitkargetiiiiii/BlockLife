...
| --- | --- | --- | --- |
| **world** (10.7k) | Grid, sector streaming, authoring kit, buildings, roads, districts, surfaces/facade art, solidity, prop visual envelopes + the deterministic parked-body mapping, the registry, WorldDirector | `runtimeRegistry.ts`, `WorldDirector.tsx`, `sectors/*`, `authoring/*`, `surfaces/*`, `solidFootprints.ts`, `propPlacement.ts`, `parkedVehicleBodies.ts`, `Buildings.tsx`, `Roads.tsx` | [LARGE_CITY_FOUNDATION](LARGE_CITY_FOUNDATION.md), [AUTHORING_KIT](DISTRICT_AUTHORING_KIT.md) |
...
| **housing** (2.3k) | 3-tier property + slot registry (validated) + PURE lease/rent model (deposit, 7-day rent, grace, one late fee, exact-once auto/manual pay) + module-singleton runtime (unique reload-safe furniture assets + placements) + 19-def furniture catalog + PURE placement validator + ONE bounded metric calculator (Comfort/Style/Storage/Sleep/Hosting, anti-inflation) + typed career/social adapters + additive fail-safe save/migration (Housing v1) | `housingTypes.ts`, `propertyRegistry.ts`, `leaseModel.ts`, `housingRuntime.ts`, `furnitureCatalog.ts`, `placement.ts`, `homeMetrics.ts`, `housingCommerce.ts`, `eligibility.ts`, `housingCareer.ts`, `housingSocial.ts`, `housingPersistence.ts`, `housingObservability.ts` | [HOUSING_FURNITURE_AND_PROPERTY](HOUSING_FURNITURE_AND_PROPERTY.md) |
| **vehicles** (Ownership v1) | 4-class validated registry (Compact == baseline) + module-singleton owned-asset runtime (`ov_<n>`, cap 4, one-active/one-per-anchor, exact-once txn) + the ONE-shell projection (tuning/footprint/paint; defaults to legacy Compact) + 11 authored parking anchors + dealership/trade-in through **commerce** + condition/repair/impound + crash→condition wear + per-asset cargo over the **inventory service** + customization (upgrades+paint) + additive fail-safe save/legacy→Compact migration + observability. Owned-vs-stolen kept separate by id namespace | `vehicleRegistry.ts`, `vehicleOwnershipTypes.ts`, `vehicleOwnershipRuntime.ts`, `vehicleOwnershipPersistence.ts`, `vehicleProjection.ts`, `parkingRegistry.ts`, `vehicleParkingService.ts`, `vehicleDealership.ts`, `vehicleEligibility.ts`, `vehicleService.ts`, `vehicleCustomization.ts`, `vehicleCargo.ts`, `vehicleOwnershipObservability.ts`, `OwnedParkedVehicles.tsx` (+ existing `VehicleController.ts`/`Vehicle.tsx`) | [VEHICLE_OWNERSHIP_PARKING_CUSTOMIZATION](VEHICLE_OWNERSHIP_PARKING_CUSTOMIZATION.md) |
...
| **characters** (1.1k) | Rigged-model pipeline: manifest, motion state, animation controller, GLB-or-primitive fallback, the population appearance registry, and the **strict 1:1 named-resident body mapping** (`WAVE4_NAMED_BODIES` — the player is never one of them) | `characterRuntime.ts`, `characterManifest.ts`, `characterAnimationState.ts`, `populationAppearance.ts`, `NpcCharacter.tsx` | [SYSTEMS §Characters](SYSTEMS.md#characters), [IDENTITY](CHARACTER_IDENTITY_AND_POPULATION.md), [WAVE 4](ASSET_INTEGRATION_WAVE_4.md) |
...
| **assets** (0.5k) | Asset manifest (incl. measured `renderedTopY` per world body), GLB landmark loader with primitive fallback, the asset-settle quiet-window policy | `assetManifest.ts`, `LandmarkAsset.tsx`, `assetSettle.ts` | [SYSTEMS §Assets](SYSTEMS.md#assets--glb-loading), [VISUAL_QUALITY](VISUAL_QUALITY_AND_BASELINE_INTEGRITY.md), [WAVE 4](ASSET_INTEGRATION_WAVE_4.md) |
